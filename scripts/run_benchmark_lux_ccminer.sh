...
RESULTFILE=${log_dir}/offline_bench_result_gpu${device_id}_LUX.dat
if [[ "$OSTYPE" == "msys" ]]
then
#MINGW
...
BENCH_LOGCMD="$(${MINER_BINARY} -a phi2 --benchmark -d ${device_id_cuda} 2>&1)"
...
avg_hashrate=$(echo "${BENCH_LOGCMD}" | grep -oP 'phi2 hashrate =\s*\K[+-]?[0-9]+([.][0-9]+)?')
