...
../../miner/windows/xmr-stak/xmr-stak.exe --noCPU --benchmark 6 --nvidia nvidia$device_id.txt &> log_${mem_clock}_${graph_clock}.txt
...
avg_hashrate=$(grep -i 'Benchmark Total:' log_${mem_clock}_${graph_clock}.txt | grep -Eo '[+-]?[0-9]+([.][0-9]+)?' | tail -n 1)
