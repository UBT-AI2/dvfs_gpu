...
namespace frequency_scaling {
    ...
    std::string log_utils::get_offline_bench_filename(const currency_type &ct, int device_id_nvml, bool logdir_prefix) {
        const std::string &prefix_str = (logdir_prefix) ? log_utils::get_logdir_name() + "/" : "";
        return prefix_str + "offline_bench_result_gpu" + std::to_string(device_id_nvml) + "_" + ct.currency_name_ +
               ".dat";
    }

    std::string log_utils::get_online_bench_filename(const currency_type &ct, int device_id_nvml, bool logdir_prefix) {
        const std::string &prefix_str = (logdir_prefix) ? log_utils::get_logdir_name() + "/" : "";
        return prefix_str + "online_bench_result_gpu" + std::to_string(device_id_nvml) + "_" + ct.currency_name_ +
               ".dat";
    }

    std::string log_utils::get_profit_stats_filename(const currency_type &ct, int device_id_nvml, bool logdir_prefix) {
        const std::string &prefix_str = (logdir_prefix) ? log_utils::get_logdir_name() + "/" : "";
