...
int main(int argc, char **argv) {
    if (argc < 6) {
        full_expression_accumulator<>(std::cout) << "Usage: " << argv[0]
                                                 << " <currency_type> <device_id> <max_iterations> <mem_step> <graph_idx_step> "
    ...
    if (argc > 6)
        min_hashrate = atof(argv[6]);
    if (argc > 7)
        min_mem_oc = atoi(argv[7]);
    if (argc > 8)
        max_mem_oc = atoi(argv[8]);
    if (argc > 9)
        min_graph_oc = atoi(argv[9]);
    if (argc > 10)
        max_graph_oc = atoi(argv[10]);
