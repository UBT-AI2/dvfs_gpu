...
int main(int argc, char **argv) {
    if (argc < 7) {
        full_expression_accumulator<>(std::cout) << "Usage: " << argv[0]
                                                 << " <currency_config> <currency_type> <device_id> <max_iterations> <mem_step_pct> <graph_idx_step_pct> "
                                                    "[<min_hashrate_pct>] [<min_mem_oc>] [<max_mem_oc>] [<min_graph_oc>] [<max_graph_oc>]"
                                                 << std::endl;
        return 1;
    }
    ...
    double mem_step_pct = atof(argv[5]);
    double graph_idx_step_pct = atof(argv[6]);
    double min_hashrate_pct = -1.0;
    int min_mem_oc = 1, max_mem_oc = -1;
    int min_graph_oc = 1, max_graph_oc = -1;
    if (argc > 7)
        min_hashrate_pct = atof(argv[7]);
    if (argc > 8)
    ...
    try {
        ...
        //
        const measurement &m = freq_hill_climbing(&run_benchmark_mining_offline, ct, dci, max_iterations, mem_step_pct,
                                                  graph_idx_step_pct, min_hashrate_pct);
