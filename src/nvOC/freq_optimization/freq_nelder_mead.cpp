...
namespace frequency_scaling {
    ...
    measurement freq_nelder_mead(const benchmark_func &benchmarkFunc, currency_type ct, const device_clock_info &dci,
        ...
        double param_tol = 0.025 * (Eigen::Vector2d(dci.min_graph_oc_, dci.min_mem_oc_) -
                                Eigen::Vector2d(dci.max_graph_oc_, dci.max_mem_oc_)).norm();
        nm_specific_options nm_opt(3, std::make_pair(1.5, 2.5), std::numeric_limits<double>::max(), param_tol);
        ...
        if (dci.nvapi_supported_)
            ...
            return freq_nelder_mead1D(benchmarkFunc, ct, dci, start_node, max_iterations, mem_step,
                                      graph_idx_step, min_hashrate, nm_opt);
        else {
