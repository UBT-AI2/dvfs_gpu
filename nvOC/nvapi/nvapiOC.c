...
int main(int argc, char **argv)
{
    ...
    int nGPU=0, idxGPU=0, userfreq = 0, systype=0, memsize=0, memtype=0;
    ...
    printf("Current RAM OC: %dMHz\n", (int)((pstates_info.pstates[0].clocks[1]).freqDelta_kHz.value)/1000);
        ...
        userfreq = atoi(argv[2])*1000;
            ...
            buf[10] = userfreq;
            NvSetPstates(hdlGPU[idxGPU], buf)? printf("\nGPU OC failed!\n") : printf("\nGPU OC OK: %d MHz\n", userfreq/1000);
            free(buf);
           
   
        userfreq = atoi(argv[3])*1000;
        
            buf = malloc(0x1c94);
            memset(buf, 0, 0x1c94);
            buf[0] = 0x11c94; buf[2] = 1; buf[3] = 1;
            buf[7] = 4; buf[10] = memtype<=7?userfreq:userfreq*2;
            NvSetPstates(hdlGPU[idxGPU], buf)? printf("VRAM OC failed!\n") : printf("RAM OC OK: %d MHz\n", userfreq/1000);
            free(buf);
